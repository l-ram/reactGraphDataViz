...
d3sparql.htmltable = function (json, config) {
  ...
  var table = d3sparql
    .select(opts.selector, "htmltable")
    .append("table")
...
d3sparql.barchart = function (json, config) {
  ...
  var scale_x = d3.scale
    .ordinal()
    .rangeRoundBands([0, opts.width - opts.margin], 0.1);
  ...
  var ax = svg
    ...
    .attr(
      "transform",
      "translate(" + opts.margin + "," + (opts.height - opts.margin) + ")"
    )
    .call(axis_x);
  var ay = svg
    .append("g")
...
d3sparql.scatterplot = function (json, config) {
  ...
  var scale_y = d3.scale
    ...
    .domain(extent_y);
  var scale_r = d3.scale
    .linear()
    .range([opts.min_r, opts.max_r])
    .domain(extent_r);
  ...
  function dragmove(d) {
    d3.select(this).attr(
      ...
      "translate(" +
        d.x +
        "," +
        (d.y = Math.max(0, Math.min(opts.height - d.dy, d3.event.y))) +
        ")"
    );
    sankey.relayout();
    link.attr("d", path);
...
d3sparql.roundtree = function (json, config) {
  ...
  var svg = d3sparql
    ...
    .attr(
      "transform",
      "translate(" + opts.diameter / 2 + "," + opts.diameter / 2 + ")"
    );
  var link = svg
    .selectAll(".link")
    .data(links)
...
d3sparql.sunburst = function (json, config) {
  ...
  function maxDepth(d) {
    return d.children
      ? Math.max.apply(Math, d.children.map(maxDepth))
      : d.y + d.dy;
  }
  function arcTween(d) {
  ...
  d3.json(opts.topojson, function (error, world) {
    ...
    svg
      ...
      .attr("transform", function (d) {
        return (
          "translate(" +
          projection([d[opts.var_lng].value, d[opts.var_lat].value]) +
          ")"
        );
      });
  });
...
d3sparql.namedmap = function (json, config) {
  ...
  d3.json(opts.topojson, function (topojson_map) {
    var geo = topojson.object(
      topojson_map,
      topojson_map.objects[opts.mapname]
    ).geometries;
    var projection = d3.geo
      .mercator()
